 ...
 */
@RouterService(interfaces = OrderRpc.class, singleton = true, defaultImpl = true)
public class OrderImpl implements OrderRpc {

    @Override
    public void makeOrder(String id) {
        User user = Router.getService(InfoRpc.class).getUser();
        String hint = user.getName() + " has make an order, order id is:" + id;
