package com.sloth.utils.dynamic;

import com.sloth.utils.dynamic.opt.Opt;
import com.sloth.utils.dynamic.opt.OptDouble;
import com.sloth.utils.dynamic.opt.OptFloat;
import com.sloth.utils.dynamic.opt.OptInt;
import com.sloth.utils.dynamic.opt.OptLong;
import com.sloth.utils.dynamic.opt.OptStr;
import com.sloth.utils.dynamic.opt.OptStr2;
...
public final class DynamicBean {
    ...
    public Integer toInt(Integer def){
        ...
        if(_obj instanceof Long){
            return ((Long)_obj).intValue();
        }

        if(_obj instanceof Float){
            return ((Float)_obj).intValue();
        }

        if(_obj instanceof Double){
            return ((Double)_obj).intValue();
        }

        if(_obj instanceof Boolean){
            return (Boolean) _obj ? 1 : 0;
        }
    ...
    public Long toLong(Long def){
        ...
        if(_obj instanceof Float){
            return ((Float)_obj).longValue();
        }

        if(_obj instanceof Double){
            return ((Double)_obj).longValue();
        }

        if(_obj instanceof Boolean){
            return (Boolean) _obj ? 1L : 0L;
        }
    ...
    public String toStr(String def){
        ...
        if(_obj instanceof Float){
            float fTp = (Float) _obj;
            if(fTp == Math.round(fTp)){
                return String.valueOf((int)fTp);
            }
        }
        if(_obj instanceof Double){
            double fTp = (Double) _obj;
            if(fTp == Math.round(fTp)){
                return String.valueOf((int)fTp);
            }
        }
