...
public final class AppUtils {
    ...
    public static void registerAppStatusChangedListener(@NonNull final Utils.OnAppStatusChangedListener listener) {
        UtilsCenter.addOnAppStatusChangedListener(listener);
    }
    ...
    public static void unregisterAppStatusChangedListener(@NonNull final Utils.OnAppStatusChangedListener listener) {
        UtilsCenter.removeOnAppStatusChangedListener(listener);
    }
    ...
    public static void installApp(final String filePath) {
        installApp(UtilsCenter.getFileByPath(filePath));
    }
    ...
    public static void installApp(final File file) {
        Intent installAppIntent = UtilsCenter.getInstallAppIntent(file);
    ...
    public static void installApp(final Uri uri) {
        Intent installAppIntent = UtilsCenter.getInstallAppIntent(uri);
        if (installAppIntent == null) return;
        Utils.getApp().startActivity(installAppIntent);
    }
    ...
    public static void uninstallApp(final String packageName) {
        if (UtilsCenter.isSpace(packageName)) return;
        Utils.getApp().startActivity(UtilsCenter.getUninstallAppIntent(packageName));
    }
    ...
    public static boolean isAppInstalled(final String pkgName) {
        if (UtilsCenter.isSpace(pkgName)) return false;
    ...
    public static boolean isAppRoot() {
        ShellUtils.CommandResult result = UtilsCenter.execCmd("echo root", true);
    ...
    public static boolean isAppDebug(final String packageName) {
        if (UtilsCenter.isSpace(packageName)) return false;
        try {
            PackageManager pm = Utils.getApp().getPackageManager();
    ...
    public static boolean isAppForeground() {
        return UtilsCenter.isAppForeground();
    }
    ...
    public static boolean isAppForeground(@NonNull final String pkgName) {
        return !UtilsCenter.isSpace(pkgName) && pkgName.equals(UtilsCenter.getForegroundProcessName());
    }
    ...
    public static void launchApp(final String packageName) {
        if (UtilsCenter.isSpace(packageName)) return;
        Intent launchAppIntent = UtilsCenter.getLaunchAppIntent(packageName);
    ...
    public static void relaunchApp(final boolean isKillProcess) {
        Intent intent = UtilsCenter.getLaunchAppIntent(Utils.getApp().getPackageName());
        if (intent == null) {
            Log.e("AppUtils", "Didn't exist launcher activity.");
            return;
        }
    ...
    public static void launchAppDetailsSettings(final String pkgName) {
        if (UtilsCenter.isSpace(pkgName)) return;
        Intent intent = UtilsCenter.getLaunchAppDetailsSettingsIntent(pkgName, true);
        if (!UtilsCenter.isIntentAvailable(intent)) return;
        Utils.getApp().startActivity(intent);
    }
    ...
    public static void launchAppDetailsSettings(final Activity activity, final int requestCode, final String pkgName) {
        if (activity == null || UtilsCenter.isSpace(pkgName)) return;
        Intent intent = UtilsCenter.getLaunchAppDetailsSettingsIntent(pkgName, false);
        if (!UtilsCenter.isIntentAvailable(intent)) return;
    ...
    public static void exitApp() {
        UtilsCenter.finishAllActivities();
        System.exit(0);
    }
    ...
    public static Drawable getAppIcon(final String packageName) {
        if (UtilsCenter.isSpace(packageName)) return null;
        try {
            PackageManager pm = Utils.getApp().getPackageManager();
    ...
    public static int getAppIconId(final String packageName) {
        if (UtilsCenter.isSpace(packageName)) return 0;
        try {
            PackageManager pm = Utils.getApp().getPackageManager();
            PackageInfo pi = pm.getPackageInfo(packageName, 0);
    ...
    public static int getAppVersionCode(final String packageName) {
        if (UtilsCenter.isSpace(packageName)) return -1;
        try {
            PackageManager pm = Utils.getApp().getPackageManager();
            PackageInfo pi = pm.getPackageInfo(packageName, 0);
    ...
    private static List<String> getAppSignaturesHash(final String packageName, final String algorithm) {
        ...
        if (UtilsCenter.isSpace(packageName)) return result;
        Signature[] signatures = getAppSignatures(packageName);
        if (signatures == null || signatures.length <= 0) return result;
        for (Signature signature : signatures) {
            String hash = UtilsCenter.bytes2HexString(UtilsCenter.hashTemplate(signature.toByteArray(), algorithm))
    ...
    public static AppUtils.AppInfo getApkInfo(final String apkFilePath) {
        if (UtilsCenter.isSpace(apkFilePath)) return null;
        PackageManager pm = Utils.getApp().getPackageManager();
        if (pm == null) return null;
