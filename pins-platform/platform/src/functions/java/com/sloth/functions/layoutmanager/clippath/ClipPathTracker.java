 ...
 */
public class ClipPathTracker implements Tracker<Path> {
    ...
    public ClipPathTracker(long duration) {
        this.duration = duration;
    }

    public ClipPathTracker(int mode, long duration) {
        this.mode = mode;
        this.duration = duration;
    }

    public ClipPathTracker(long duration, int width, int height) {
        this.width = width;
        this.height = height;
        this.duration = duration;
    }

    public ClipPathTracker(int mode, long duration, int width, int height) {
        this.mode = mode;
        this.duration = duration;
        this.width = width;
        this.height = height;
    ...
    @Override
    public Path track(float fraction) {
        Path path = new Path();
    ...
    private void assignCenter(float fraction) {
        if(!Float.valueOf(fraction).equals(0.0f)){
            return;
        }


        Random rd = new Random(System.currentTimeMillis());
        centerX = Math.abs(rd.nextInt(width));
        centerY = Math.abs(rd.nextInt(height));
