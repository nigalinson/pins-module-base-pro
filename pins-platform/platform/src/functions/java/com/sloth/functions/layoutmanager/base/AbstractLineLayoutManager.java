 ...
 * 抽象线性排版器
 */
public abstract class AbstractLineLayoutManager extends LinearLayoutManager implements PositionScrollable {
     ...
     */
    public AbstractLineLayoutManager(Context context) {
     ...
     */
    public AbstractLineLayoutManager(Context context, int orientation, boolean reverseLayout) {
        super(context);
    ...
    /**
     * Sets the orientation of the layout. {@link AbstractLineLayoutManager}
    ...
    private void layoutChildDetectAvailable(RecyclerView.Recycler recycler, RecyclerView.State state) {
        try{
    ...
    public boolean onAddFocusables(RecyclerView recyclerView, ArrayList<View> views, int direction, int focusableMode) {
        ...
        if (recyclerView.hasFocus()) {
            ...
            if (movement != DIRECTION_NO_WHERE) {
                ...
                ScrollHelper.smoothScrollToPosition(recyclerView, targetPosition);
            }
        } else {
    ...
    public void scrollToPosition(int position) {
        if (!getInfinite() && (position < 0 || position >= getItemCount())) {
            return;
        }
    ...
    private int scrollBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        //handle the boundary
        if (!getInfinite() && targetOffset < getMinOffset()) {
            willScroll -= (targetOffset - getMinOffset()) * getDistanceRatio();
        } else if (!getInfinite() && targetOffset > getMaxOffset()) {
    ...
    public int distanceToTargetPosition(int targetPosition) {
        return getOffsetToPosition(targetPosition);
    }
    ...
    public boolean getInfinite() {
        return mInfinite && getItemCount() > 1;
    }
