...
public class CacheMemoryUtilsTest extends BaseTest {
    ...
    public void get() {
        for (int i = 0; i < 10; i++) {
            assertEquals(Integer.valueOf(i), mCacheMemoryUtils1.get(String.valueOf(i)));
        }
        for (int i = 0; i < 10; i++) {
            if (i < 7) {
                assertNull(mCacheMemoryUtils2.get(String.valueOf(i)));
            } else {
                assertEquals(Integer.valueOf(i), mCacheMemoryUtils2.get(String.valueOf(i)));
            }
        }
    }
    ...
    public void getExpired() throws Exception {
        mCacheMemoryUtils1.put("10", 10, 2 * CacheConstants.SEC);

        assertEquals(Integer.valueOf(10), mCacheMemoryUtils1.get("10"));
        Thread.sleep(1500);
        assertEquals(Integer.valueOf(10), mCacheMemoryUtils1.get("10"));
        Thread.sleep(1500);
